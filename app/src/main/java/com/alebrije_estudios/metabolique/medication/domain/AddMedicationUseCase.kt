...
class AddMedicationUseCase @Inject constructor(private val repository: MedicationRepository) {
    suspend operator fun invoke(authData:AuthData,localDate: LocalDate,medication: MedicationModel) = repository.addMedication(authData.token, medication.toEntityPost(authData,localDate))
}